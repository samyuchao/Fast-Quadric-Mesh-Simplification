...
void Simplify::simplify_mesh(int target_count, double agressiveness, bool verbose)
{
	// init
...
void Simplify::simplify_mesh_lossless(bool verbose)
{
	// init
	loopi(0, triangles.size()) triangles[i].deleted = 0;
...
double Simplify::min(double v1, double v2) {
	return fmin(v1, v2);
}
...
//Option : Load OBJ
void Simplify::load_obj(const char* filename) {
	vertices.clear();
	triangles.clear();
...
void Simplify::write_obj(const char* filename)
{
...
void Simplify::AddVertex(double x, double y, double z)
{
	Vertex v;
	v.p.x = x;
	v.p.y = y;
...
void Simplify::AddTriangle(int v0, int v1, int v2)
{
	Triangle t;
	t.v[0] = v0;
	t.v[1] = v1;
...
void Simplify::GetVertex(int index, vec3f &a)
{
...
void Simplify::GetTriangle(int index, std::vector<int> &v)
{
	Triangle t = triangles.at(index);
	v.push_back(t.v[0]);
	v.push_back(t.v[1]);
	v.push_back(t.v[2]);
}
